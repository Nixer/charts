...
from utils.last_friday import last_friday
...
def create_app():
    ...
    def index():
        if request.method == 'POST':
            ...
            month = data['month']
            year = data['year']
            friday = last_friday(f"{year}-{month}-{day}")
            current_top_us = []
            current_top_uk = []
            chart_for_year_uk = UkChart.query.filter_by(date=friday).order_by(UkChart.rank).all()
            chart_for_year_us = UsChart.query.filter_by(date=friday).order_by(UsChart.rank).all()
            for c in chart_for_year_uk:
                track = Track.query.get(c.track_id)
                current_top_uk.append({'title': track.title, 'artist': track.artist, 'rank': c.rank})
            for c in chart_for_year_us:
                track = Track.query.get(c.track_id)
                current_top_us.append({'title': track.title, 'artist': track.artist, 'rank': c.rank})
            # return f"{friday}: {charts}"
            return render_template('index.html', days=days, years=years,
